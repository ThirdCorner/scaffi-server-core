...
var ComponentLoader = function () {
	...
	_createClass(ComponentLoader, [{
		...
		value: function loadCoreComponents() {
			...
			this.getFiles(__dirname, function (file) {
				...
				console.log("HAS FILE: " + file, _lodash2.default.has(that.config.components, file));
   ...
   */
		...
		key: "loadDependancy",
		value: function loadDependancy(name, isComponent) {

			var extended;
			if (!isComponent) {
				extended = require(_path2.default.join(this.componentsDir, name));
			}
			try {
				extended = require(_path2.default.join(this.customComponents, name));
		...
		value: function moveComponentToBase(name) {
			...
			if (_lodash2.default.isFunction(component.requiredComponents)) {
		...
		value: function checkRequiresExistence(component, name) {
			...
			var requiredComponents = _lodash2.default.isFunction(component.requiredComponents) && _lodash2.default.isArray(component.requiredComponents()) ? component.requiredComponents() : [];
			_lodash2.default.each(requiredComponents, function (depName) {
