...
var App = function (_AbstractComponent) {
	...
	_createClass(App, [{
		...
		value: function setupAppPresets() {
			...
			var whitelist = ['http://localhost:4000', 'http://localhost:4001', 'http://localhost:' + port.toString(), "file://"];
			var corsOptions = {
				origin: function origin(_origin, callback) {
					var originIsWhitelisted = whitelist.indexOf(_origin) !== -1;
					var msg = 'Bad Request';
					if (_this2.getConfig("environment") == "development" || _this2.getConfig("environment") == "localhost" || _this2.getConfig("environment") == "prototype") {
    ...
    */
			...
			//app.use(logger('dev'));
			app.use(_bodyParser2.default.json({ limit: this.getConfig('limit') || '50mb' }));
			app.use(_bodyParser2.default.urlencoded({ limit: this.getConfig('limit') || '50mb', extended: false }));
